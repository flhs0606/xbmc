...
void CCurlFileEngine::WorkerLoop()
{
  ...
  while (m_running) {
    ...
    // ★ HTTP 4xx handling — must come BEFORE res==CURLE_OK because HTTP errors
    //    return res==CURLE_OK (the transfer succeeded, the server just said 403).
    //    Without this check, 4xx quietly sets m_eof=true which causes "stuck loading".
    if (res == CURLE_OK && code >= 400 && code < 500 && code != 429) {
      // ★ CDN signed URL may have limited-use token (e.g. c=2 in 115cdn URL).
      //    If we're using a cached CDN URL, clear it and retry with the original
      //    URL to get a fresh 302→CDN redirect, rather than failing permanently.
      //    Use a separate counter to prevent infinite fallback loops.
