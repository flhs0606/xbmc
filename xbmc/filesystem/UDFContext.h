...
public:
  ...
  /*!
   ...
   The volume is unmounted once the last holder has let go of it and a short grace period has
   passed, so that the run of opens examining a disc costs makes it through - the menu probing
   for index.bdmv, the directory listing the playlists, the player opening the disc - without
   re-mounting each time. Holding a mount across a run of opens with a CUDFMount still saves the
   mount of the first one.
