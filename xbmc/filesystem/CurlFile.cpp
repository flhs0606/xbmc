...
bool CCurlFile::Open(const CURL& url)
{
  ...
  if(url2.IsProtocol("http") || url2.IsProtocol("https"))
  {
    ...
    // (IMG, NRG, UDF — ISO goes through CCurlFileEngine which is single-connection
    // by design). Legacy CReadState multisession can create concurrent Range
    // connections that exhaust limited-use CDN tokens (e.g. c=2 per URL).
    // Disabling it here prevents 403 rate-limit errors on the legacy path.
