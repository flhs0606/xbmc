...
{
  const PlayItemInformation* longest{nullptr};
  auto longestDuration{std::chrono::milliseconds::zero()};
  for (const PlayItemInformation& playItem : playlist.playItems)
  {
    const auto duration{playItem.outTime - playItem.inTime};
    if (!longest || duration > longestDuration)
    {
      longest = &playItem;
